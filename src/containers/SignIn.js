...
import { queryServerApi } from "../util/queryServerApi";
...
class SignIn extends React.Component {
  ...
  render() {
    const { email, password, user, a, b } = this.state;
    const go = async () => {
      try {
        ...
        if (userPosts.data[0].Role === "Admin") {
          console.log(userPosts.data[0]);
          this.props.history.push("/app/dashboard/crypto");
        } else if (userPosts.data[0].Role === "Driver") {
